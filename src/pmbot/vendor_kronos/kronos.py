...
def auto_regressive_inference(tokenizer, model, x, x_stamp, y_stamp, max_context, pred_len, clip=5, T=1.0, top_k=0, top_p=0.99, sample_count=5, verbose=False, return_all=False):
    with torch.no_grad():
        ...
        preds = z.cpu().numpy()
        if return_all:
            return preds  # (batch, sample_count, pred_len, feat)：保留全部独立样本
...
class KronosPredictor:
    ...
    def generate(self, x, x_stamp, y_stamp, pred_len, T, top_k, top_p, sample_count, verbose, return_all=False):
        x_tensor = torch.from_numpy(np.array(x).astype(np.float32)).to(self.device)
        x_stamp_tensor = torch.from_numpy(np.array(x_stamp).astype(np.float32)).to(self.device)
        y_stamp_tensor = torch.from_numpy(np.array(y_stamp).astype(np.float32)).to(self.device)
        preds = auto_regressive_inference(self.tokenizer, self.model, x_tensor, x_stamp_tensor, y_stamp_tensor, self.max_context, pred_len,
                                          self.clip, T, top_k, top_p, sample_count, verbose, return_all=return_all)
        if return_all:
            return preds
        preds = preds[:, -pred_len:, :]
        return preds

    def predict(self, df, x_timestamp, y_timestamp, pred_len, T=1.0, top_k=0, top_p=0.9, sample_count=1, verbose=True, return_all=False):
        ...
        preds = self.generate(x, x_stamp, y_stamp, pred_len, T, top_k, top_p, sample_count, verbose, return_all=return_all)
        if return_all:
            # (1, sample_count, pred_len, feat) → 展开为 sample_count×pred_len 行
            preds = preds.squeeze(0)
            n_rows = preds.shape[0] * preds.shape[1]
            preds = preds.reshape(n_rows, -1)
            preds = preds * (x_std + 1e-5) + x_mean
            idx = np.repeat(np.asarray(y_timestamp), preds.shape[0] // len(np.asarray(y_timestamp)))
            pred_df = pd.DataFrame(preds, columns=self.price_cols + [self.vol_col, self.amt_vol], index=idx)
            return pred_df
        preds = preds.squeeze(0)
        preds = preds * (x_std + 1e-5) + x_mean
