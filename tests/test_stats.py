...
def test_empty_trades_gives_zero_stats(tmp_path):
    stats = compute_stats(trades_path=Path(tmp_path) / "empty.csv", accuracy={"correct": 0, "total": 0, "accuracy": 0.0})
...
def test_win_rate_and_roi(tmp_path):
    ...
    make_trades(p, rows)
    stats = compute_stats(trades_path=p, accuracy={"correct": 3, "total": 5, "accuracy": 0.6})
...
def test_accuracy_come_from_prediction_log(tmp_path):
    ...
    stats = compute_stats(trades_path=p, accuracy={"correct": 10, "total": 20, "accuracy": 0.5})
...
def test_validation_threshold_trades(tmp_path):
    ...
    make_trades(p, rows)
    stats = compute_stats(trades_path=p, accuracy={"correct": 0, "total": 0, "accuracy": 0.0})
...
def test_report_contains_metrics_and_config(tmp_path):
    p = Path(tmp_path) / "trades.csv"
    make_trades(p, [trade_row("2026-08-01T00:00:00+00:00", +0.55)])
    stats = compute_stats(trades_path=p, accuracy={"correct": 1, "total": 2, "accuracy": 0.5})
